...
void setup() {
    ...
    accelgyro.setYAccelOffset(371);
    accelgyro.setZAccelOffset(1997);
...
void loop() {
    ...
    madwick_filter.updateIMU(gxr, gyr, gzr, ayg, axg, azg);
    ...
    if (millis() - tmr > INTERVAL) {
      ...
      sprite.printf("pitch=%3.1f\n", madwick_filter.getPitch());
      sprite.printf("roll=%3.1f\n", madwick_filter.getRoll());
      sprite.printf("yaw=%3.1f", madwick_filter.getYaw());
